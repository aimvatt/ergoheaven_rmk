...
# v3 = minimal v2 native-scroll routing + conservative low-latency touch timing.
# No custom 3-finger gesture state machine is added here.

touch_path = Path("keyboards/k04/src/touchpad.rs")
touch = touch_path.read_text()

# Reduce wake/first-contact latency while keeping the active 8 ms cadence unchanged.
touch = replace_once(
    touch,
...
touch = replace_once(
    ...
    "const TOUCH_SCROLL_CONFIRM_SAMPLES: u8 = 2;",
    "two-finger confirmation",
)

# Keep stock touch/gesture state machine. Only native scroll transport changes.
touch = replace_once(
    touch,
...
print("K:04 custom touchpad v3 low-latency patch applied successfully")
